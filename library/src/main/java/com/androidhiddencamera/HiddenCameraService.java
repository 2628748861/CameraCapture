...
public abstract class HiddenCameraService extends Service implements CameraCallbacks {
     ...
     * initialize the camera using before using this function.
     */
    ...
    protected void takePicture(CameraConfig cameraConfig) {
        if (mCameraPreview != null) {
            if (mCameraPreview.isSafeToTakePictureInternal()) {
                mCameraPreview.takePictureInternal(cameraConfig.getImageFiles());
            }
        } else {
            throw new RuntimeException("Background camera not initialized. Call startCamera() to initialize the camera.");
        }
